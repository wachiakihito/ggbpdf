...
class HLR {
  ...
  // 以下はthisのインスタンス変数
  // 入力 eyex      視点の位置 (eye, 0, 0) から (0,0,0) を見る
  //      scrnx     スクリーンの位置。x = scrnx
  //      gap       線分どうしが重なったときに欠く幅。単位は3次元座標での距離1。
  //                欠かないならnull
  //      segs3d    空間の線分の配列
  //      poly3d    空間の多角形の配列。辺は描画されないのでsegs3dに入れること。
  //      s_on_p    線分番号=>[それを含む面番号]の配列
  //      segstyles 線分のスタイルの配列
  // 出力 vsegs2d   見える線分の配列。切り欠き済み
  //      vstyles   見える線分のスタイルの連想配列
  //      isegs2d   見えない線分の配列
  //      istyles   見える線分のスタイルの連想配列
  visible_segs() {
    // 視点
