...
- [1. ggbpdf.ggb](#1-ggbpdfggb)
  ...
  - [1-2. 使用方法](#1-2-使用方法)
  - [1-3. 対応している図形・機能](#1-3-対応している図形・機能)
  - [1-4. 対応していない図形・機能](#1-4-対応していない図形・機能)
  - [1-5. 保存](#1-5-保存)
  - [1-6. htmlにエクスポート](#1-6-htmlにエクスポート)
- [2. ggbpdf.html](#2-ggbpdfhtml)
  - [2-1. スクリーンショット](#2-1-スクリーンショット)
  - [2-2. 使用方法、対応している図形・機能、対応していない図形・機能](#2-2-使用方法、対応している図形・機能、対応していない図形・機能)
  - [2-3. 保存](#2-3-保存)
  - [2-4. htmlにエクスポート](#2-4-htmlにエクスポート)
...
* **ggbpdf.ggb**: ggbファイル。svgを出力できる。
* **ggbpdf.html**: htmlにエクスポート [^1] されたGeoGebra。svg, pdfを出力できる。
...
### 1-4. 対応していない図形・機能
* 作成されるsvg, pdf画像のサイズは360x360で固定です。
* 点のラベルで表示できるのは名前また見出しです。
見出しにtex数式を用いた場合、svgにはタイプセットされずに表示されます。
...
### 2-3. 保存
ggbファイルに保存すると、**ggbpdf.ggb**と同様、svg画像出力機能を持ったまま保存できます。
pdf画像出力機能は使えなくなりますが、再び「htmlにエクスポート」すると、使えるようになります。

### 2-4. htmlにエクスポート
「htmlにエクスポート」すると、
svg, pdf画像出力機能を保ったまま保存ができます。


## 3. ggbtex.rb
ネットワーク接続なしでも使え、GeoGebraを起動する必要もなく、
texファイルを出力するのはこのスクリプトのみであるため、一応残してありますが、
今後メンテナンスはしない予定です。
...
2. **ggbpdf.ggb**、**ggbpdf.html**とも、
pdf画像についてはsvg画像と違って、線分の太さなどのスタイルを反映しません。
そのうち対応します。
